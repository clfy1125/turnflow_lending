...
import { CheckCircle2, ArrowRight, Heart, Send, Bookmark, MessageSquare, MoreHorizontal, ChevronLeft, Phone, Video, Menu, Zap, Search, Mail } from "lucide-react";
import { ImageWithFallback } from "@/app/components/figma/ImageWithFallback";
import lemonJellyImage from '@/assets/8423cbfb13b3c6258199cd5f9f191987775a0784.png';
import { useState, useEffect } from "react";

export function KeywordDetectionSection() {
  const [stage, setStage] = useState(0); // 0: 메인 피드, 1: 댓글창, 2: DM창

  useEffect(() => {
    const interval = setInterval(() => {
      setStage((prev) => {
        if (prev === 0) return 1; // 메인 → 댓글창
        if (prev === 1) return 2; // 댓글창 → DM창
        return 0; // DM창 → 메인
      });
    }, 4000); // 4초로 변경하여 답글 애니메이션 시간 확보

    return () => clearInterval(interval);
  }, []);

  const comments = [
    { username: "user_a", text: "정보", timestamp: "방금", reply: "댓글 감사합니다 🙏 비타민 젤리 공구 링크 오픈되면 DM으로 보내드릴게요!" },
    { username: "customer_b", text: "정보 받기", timestamp: "1분", reply: "공구 시작되면 알림 드리겠습니다! 조금만 기다려주세요 💛" },
    { username: "lovely_c", text: "정보주세요!", timestamp: "2분", reply: "네! 공구 오픈 알림 DM 보내드릴게요 ✨" }
  ];
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <p className="text-lg text-[#6E6E6E] mb-10">
              사용자가 지정한 키워드를 댓글에서 자동으로 감지하고<br />
              실시간으로 맞춤형 DM을 발송합니다.
            </p>
            ...
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-lg mb-1">실시간 키워드 감지</h3>
                  <p className="text-[#6E6E6E]">
                    "정보", "참여", "링크" 등 원하는 키워드를 설정하면 댓글을 실시간으로 모니터링합니다
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-lg mb-1">순차적 개인화 답글</h3>
                  <p className="text-[#6E6E6E]">
                    각 댓글에 개인화된 답글을 자동으로 작성하여 고객과의 소통을 강화합니다
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-lg mb-1">즉시 DM 자동 발송</h3>
                  <p className="text-[#6E6E6E]">
                    댓글 확인 후 즉시 개인 DM으로 상세 정보를 전달하여 전환율을 높입니다
                  </p>
                </div>
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="px-3 pb-1">
                        <div className="text-xs font-semibold text-black">
                          Liked by <span className="font-semibold">marketing_pro</span> and <span className="font-semibold">3,247 others</span>
                        </div>
                      </div>
                      ...
                      <div className="px-3 pb-3">
                        ...
                        <div className="space-y-2">
                          <div className="flex items-start gap-2">
                            <div className="flex-1">
                              <div className="text-xs">
                                <span className="font-semibold text-black">user_a</span>
                                <span className="text-black ml-1">정보</span>
                              </div>
                            </div>
                          ...
                          <div className="flex items-start gap-2">
                            <div className="flex-1">
                              <div className="text-xs">
                                <span className="font-semibold text-black">customer_b</span>
                                <span className="text-black ml-1">정보 받기</span>
                              </div>
                            </div>
                            <Heart className="w-3 h-3 text-gray-400" />
                          </div>
                          <div className="flex items-start gap-2">
                            <div className="flex-1">
                              <div className="text-xs">
                                <span className="font-semibold text-black">lovely_c</span>
                                <span className="text-black ml-1">정보주세요!</span>
                              </div>
                            </div>
                            <Heart className="w-3 h-3 text-gray-400" />
                          </div>
                        ...
                        <div className="text-xs text-gray-400 mt-2">30분 전</div>
                      </div>
                              ...
                              >
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <button className="text-xs text-gray-400 font-medium">답글 달기</button>

                                    {/* Auto Reply Animation */}
                                    <motion.div
                                      initial={{ opacity: 0, height: 0, marginTop: 0 }}
                                      animate={{ opacity: 1, height: "auto", marginTop: 8 }}
                                      transition={{ duration: 0.4, delay: 0.5 + (idx * 0.3) }}
                                      className="ml-0 mt-2 flex items-start gap-2"
                                    >
                                      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-pink-500 to-orange-400 flex-shrink-0" />
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                          <span className="text-xs font-semibold text-black">turnflow_official</span>
                                          <span className="text-[10px] text-gray-400">방금</span>
                                          <span className="text-[10px] text-gray-400">· 작성자</span>
                                        </div>
                                        <p className="text-xs text-black">{comment.reply}</p>
                                      </div>
                                    </motion.div>
                                  </div>
                    ...
                    >
                      ...
                      <div className="px-3 py-2.5 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-pink-500 to-orange-400" />
                          <span className="text-sm font-semibold text-black">user_a</span>
                        </div>
                      ...
                      {/* DM Content - 동일한 aspect-square 영역 유지 */}
                      <div className="flex-1 relative">
                        <div className="absolute inset-0 p-3 overflow-y-auto">
                          <div className="space-y-3">
                            ...
                            >
                              <div className="bg-gray-100 rounded-3xl rounded-tl-sm px-3 py-2 max-w-[80%]">
                                <p className="text-xs text-black leading-relaxed">
                                  기다려 주셔서 감사해요! 한정수량 비타민 폭탄 젤리 구매 링크를 보내드립니다! 🤗
                                </p>
                              </div>
                            </motion.div>
                            ...
                            <motion.div
                              ...
                              className="bg-gray-50 rounded-xl p-2.5 border border-gray-200"
                            >
                              <div className="relative w-full" style={{ paddingBottom: '75%' }}>
                                <ImageWithFallback
                                  src="https://images.unsplash.com/photo-1767789523307-68e46587efee?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxvcmFuZ2UlMjBzbGljZXMlMjBjaXRydXMlMjBmcnVpdHxlbnwxfHx8fDE3NjgzNzcxNzV8MA&ixlib=rb-4.1.0&q=80&w=1080"
                                  alt="비타민 젤리"
                              ...
                              <div className="space-y-1.5 mt-2">
                                ...
                                <p className="text-[10px] text-gray-600 leading-relaxed">
                                  요즘 하루 종일 피곤하고 자꾸 입맛이 없거나, 피부가 푸석하다면?? 비타민 C 부족일 수 있어요!
                                </p>
                                <button className="w-full bg-white border border-gray-300 text-center py-2 rounded-lg text-xs font-medium text-[#1A6AFF] hover:bg-gray-50 transition-colors">
                                  젤리 사러 가기
                                </button>
                              </div>
                            </motion.div>
